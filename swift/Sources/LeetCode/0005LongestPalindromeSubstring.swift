public func longestPalindromeSubstring(_ s: String) -> String {
    ...
    for i in 0..<s.count {
        ...
        let (l1, r1) = expandAroundCenter(s, left: i, right: i)
        let len1 = r1 - l1 + 1
        if len1 > maxLength {
            maxLength = len1
            start = l1
        ...
        let (l2, r2) = expandAroundCenter(s, left: i, right: i+1)
        let len2 = r2 - l2 + 1
        if len2 > maxLength {
            maxLength = len2
            start = l2
...
func expandAroundCenter(_ s: String, left: Int, right: Int) -> (Int, Int) {
    var left = left;
    var right = right;
